...
if addendum:
  if MobileExist:
    ...
    ips_for_ck = spark.read.format("parquet").option("basePath", base_path).load(base_path+\
                          ...
                          "processing/mobile/flat_ip_withcount/ipCountryCode="+ipCountryCode+"/proc_date="+past_date_good.strftime('%Y-%m-%d'))\
                          .select("ip"),"ip")
...
if addendum:
  if MobileExist:
    past_date2 = past_date_good - timedelta(days=daysago)
    df_1 = spark.read.format("parquet").option("basePath", base_path).load( base_path + "input/mobile/aggregated_data")\
                    .where("date>='"+past_date_lookback.strftime('%Y-%m-%d')+"' and date<'"+past_date_good.strftime('%Y-%m-%d')+"'")\
    ...
    df_1bis = spark.read.format("parquet").option("basePath", base_path).load( base_path + "input/mobile/aggregated_data")\
                    .where("date>='"+past_date.strftime('%Y-%m-%d')+"' and date<='"+today.strftime('%Y-%m-%d')+"'")\
                    .join(flat_ip, ["ip","ipCountryCode"])\
                    .select("ifa").distinct()\
                    .withColumn("tonotuse", f.lit(1))
    ...
    df = df_1.join(df_1bis,["ifa"],"left").where("tonotuse is null").drop("tonotuse").select("ip","ipCountryCode","ifa","week","num_events","flag_ck")\
       .union(df_2).withColumnRenamed("ifa","id").withColumn("proc_date",f.lit(today.strftime('%Y-%m-%d')))

    itisNew=False
    try:
      df = spark.read.parquet(base_path + \
                      "processing/mobile/weekwindow/grouped_data_total_addendum/ipCountryCode="+ipCountryCode+"/proc_date="+today.strftime('%Y-%m-%d'))
    except:
      itisNew=True
    if itisNew:
      df.write.partitionBy("ipCountryCode","proc_date").format("parquet").mode("append")\
              .option("replaceWhere", "ipCountryCode = '"+ipCountryCode+"'")\
              .save( base_path + "processing/mobile/weekwindow/grouped_data_total_addendum/")
...
if addendum:
  if MobileExist:
    ...
    itisNew=False
    try:
      df = spark.read.parquet(base_path + "processing/mobile/weekwindow/friend_connections_addendum/ipCountryCode="+ipCountryCode+"/proc_date="+today.strftime('%Y-%m-%d'))
    except:
      itisNew=True
    if itisNew:
      friend_connections.write.partitionBy("ipCountryCode","proc_date").format("parquet").mode("append") \
      .option("replaceWhere", "ipCountryCode = '"+ipCountryCode+"'").save(base_path + "processing/mobile/weekwindow/friend_connections_addendum/")
...
if addendum:
  df = spark.read.parquet(base_path + "processing/mobile/weekwindow/friend_connections_addendum/ipCountryCode="+ipCountryCode+"/proc_date="+today.strftime('%Y-%m-%d'))
  df.select("dev1","flag_ck_dev1").where("flag_ck_dev1=0").union(df.select("dev2","flag_ck_dev2").where("flag_ck_dev2=1")).select("dev1").distinct().count()
...
if addendum:
  if MobileExist:
    itisNew=False
    try:
      df = spark.read.parquet(base_path + "processing/mobile/weekwindow/df_pair_addendum/ipCountryCode="+ipCountryCode+"/proc_date="+today.strftime('%Y-%m-%d'))
    except:
      itisNew=True
    if itisNew:
      df_pair = spark.read.format("parquet").option("basePath", base_path).load(base_path + \
                    "processing/mobile/weekwindow/friend_connections_addendum/ipCountryCode="+ipCountryCode+"/proc_date="+today.strftime('%Y-%m-%d'))\
...
if addendum:
  if MobileExist:
    ...
    if itisNew:
      ...
      .option("replaceWhere", "ipCountryCode = '"+ipCountryCode+"'").save(base_path + "processing/mobile/weekwindow/df_pair_addendum/")
