...
if addendum: 
  if MobileExist:
    df = spark.read.format("parquet").option("basePath", base_path).load(base_path + \
                    "processing/mobile/weekwindow/df_final_addendum/ipCountryCode="+ipCountryCode+"/proc_date="+today.strftime('%Y-%m-%d'))
...
if addendum:
  df = spark.read.format("parquet").option("basePath", base_path).load(base_path + \
                    "processing/mobile/weekwindow/df_final_addendum/ipCountryCode="+ipCountryCode+"/proc_date="+today.strftime('%Y-%m-%d'))
  df.where("flag_ck_dev1=0").select("dev1").union(df.where("flag_ck_dev2=0").select("dev2")).select("dev1").distinct().count()
...
if addendum:
  if MobileExist:
    itisNew=False
    try:
      df = spark.read.parquet(base_path + "processing/individual/weekwindow/lookup_couples_hash_addendum/ipCountryCode="+ipCountryCode+"/proc_date="+today.strftime('%Y-%m-%d'))
    except:
      itisNew=True
    if itisNew:
      predicted.withColumn("level",f.when(f.col("proba")>=0.95, f.lit("high"))\
                    ...
                    .option("replaceWhere", "ipCountryCode = '"+ipCountryCode+"'").save(base_path + "processing/individual/weekwindow/lookup_couples_hash_addendum")
...
if addendum:
  if MobileExist:
    dfWithHash= spark.read.format("parquet").option("basePath", base_path).load(base_path + \
              "processing/individual/weekwindow/lookup_couples_hash_addendum/ipCountryCode="+ipCountryCode+"/proc_date="+today.strftime('%Y-%m-%d'))
  
    itisNew=False
    try:
      df = spark.read.parquet(base_path + "processing/individual/weekwindow/lookup_id_hash_addendum/ipCountryCode="+ipCountryCode+"/proc_date="+today.strftime('%Y-%m-%d'))
    except:
      itisNew=True
    if itisNew:
    ...
    .option("replaceWhere", "ipCountryCode = '"+ipCountryCode+"'").save(base_path + "processing/individual/weekwindow/lookup_id_hash_addendum/")
