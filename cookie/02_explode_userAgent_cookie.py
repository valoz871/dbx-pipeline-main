...
# librerire necessarie ma che si possono installare piuttosto che lanciarle in una cella
# (seconda cella con 10K righe di codice, non aprire)
...
class UserAgent(object):
    ...
    def __init__(self, user_agent_string):
        ua_dict = Parse(user_agent_string) #user_agent_parser.Parse(user_agent_string)
...
parse_udf = f.udf(lambda x: user_agent_parser(x), ArrayType(StringType()))
...
temp = spark.read.format("parquet").load(base_path + "input/cookie/aggregated_data")\
                ...
                .groupBy("id5UID","userAgent","ipCountryCode")\
...
parsed = spark.read.format("parquet").load( base_path + "input/cookie/aggregated_data")\
                .where("date>='"+past_date.strftime('%Y-%m-%d')+"' and date<='"+today.strftime('%Y-%m-%d')+"'")\
                .where("id5UID is not null")\
                .where("userAgent is not null")\
                .where("ipCountryCode='"+ipCountryCode+"'")\
                .select("userAgent","ipCountryCode").distinct()\
                .withColumn("parsed",parse_udf(f.col("userAgent")))\
                .withColumn("proc_date",f.lit(today.strftime('%Y-%m-%d')))

itisNew=False
try:
  df = spark.read.parquet(base_path + "processing/cookie/ua_parsed_lookup/ipCountryCode="+ipCountryCode+"/proc_date="+today.strftime('%Y-%m-%d'))
except:
  itisNew=True

if itisNew:
  parsed.write.format("parquet").partitionBy("ipCountryCode","proc_date").mode("append")\
  .option("replaceWhere", "ipCountryCode = '"+ipCountryCode+"'").save(base_path + "processing/cookie/ua_parsed_lookup/")

# COMMAND ----------

ua_lookup = spark.read.format("parquet").option("basePath", base_path).load(base_path + \
                "processing/cookie/ua_parsed_lookup/ipCountryCode="+ipCountryCode+"/proc_date="+today.strftime('%Y-%m-%d'))\
                  .withColumn("browser_family",f.col("parsed")[0])\
                  .withColumn("os_family",f.col("parsed")[1])\
                  .withColumn("device_type",f.col("parsed")[2])

# COMMAND ----------

df = temp.join(ua_lookup,["userAgent","ipCountryCode"])

# COMMAND ----------



w = Window.partitionBy(f.col("id5UID"),f.col("ipCountryCode")).orderBy(f.desc("count"))

df_browser = df.where("browser_family is not null")\
              .groupBy("id5UID", "browser_family","ipCountryCode")\
              .agg(f.sum("count").alias("count"))\
              .withColumn("rn", f.row_number().over(w))\
              .where("rn == 1")\
              .drop("rn", "count")\
              .withColumnRenamed("browser_family", "mf_browser")\
              .distinct()

df_2 = df.withColumnRenamed("id_2", "id5UID").drop("browser_family")\
                    .join(df_browser, ["id5UID","ipCountryCode"], how="left")\
                    .groupBy("id5UID", "os_family", "device_type", "mf_browser","ipCountryCode")\
                    .agg(f.sum("count").alias("num_events"))
...
df_os = df_2.where("os_family is not null")\
              .groupBy("id5UID", "os_family","ipCountryCode")\
              .agg(f.sum("num_events").alias("count"))\
              .withColumn("rn", f.row_number().over(w))\
              .where("rn == 1")\
              .drop("rn", "count")\
              .withColumnRenamed("os_family", "mf_os")\
              .distinct()

df_3 = df_2.drop("os_family").join(df_os, ["id5UID","ipCountryCode"], how="left")\
                    .groupBy("id5UID","mf_os", "device_type", "mf_browser","ipCountryCode")\
                    .agg(f.sum("num_events").alias("num_events"))
...
df_device = df_3.where("device_type is not null")\
              .groupBy("id5UID", "device_type","ipCountryCode")\
              .agg(f.sum("num_events").alias("count"))\
              .withColumn("rn", f.row_number().over(w))\
              .where("rn == 1")\
              .drop("rn", "count")\
              .withColumnRenamed("device_type", "mf_device_type")\
              .distinct()

df_final = df_3.drop("device_type").join(df_device, ["id5UID","ipCountryCode"], how="left")\
                    .groupBy("id5UID", "mf_os", "mf_device_type", "mf_browser","ipCountryCode")\
                    .agg(f.sum("num_events").alias("num_events"))
...
itisNew=False
try:
  df = spark.read.parquet(base_path + "processing/cookie/id2_ua_info_clean/ipCountryCode="+ipCountryCode+"/proc_date="+today.strftime('%Y-%m-%d'))
except:
  itisNew=True

if itisNew: 
  df_final.withColumn("proc_date",f.lit(today.strftime('%Y-%m-%d'))).write.partitionBy("ipCountryCode","proc_date").format("parquet").mode("append")\
    .option("replaceWhere", "ipCountryCode = '"+ipCountryCode+"'").save(base_path + "processing/cookie/id2_ua_info_clean/")
